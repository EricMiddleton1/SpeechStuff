...
def compute_formants(samples):
  order = 12
  a = lpc.lpc.autocor(samples, order)

  roots = np.roots(a.numerator)
  realRoots = []
  for root in roots:
    #only take the roots with positive imaginary component
    if root.imag > 0:
      realRoots.append(root)

  angles = []
  for root in realRoots:
    #Compute angle of complex root and scale to frequency range
    angles.append(np.arctan2(root.imag, root.real) * RATE / (2*np.pi))

  #Sort formant frequencies into correct order
  angles.sort()
...
def lpc_spectrum(data):
    ...
    formants = compute_formants(data)
    
    formantTable.append(formants)
    if len(formantTable) >= 10:
      for formantEntry in formantTable:
        print formantEntry
      formantTable = []
      print 
